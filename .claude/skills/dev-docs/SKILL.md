...
- `git add docs/active/<nazwa>/` **+ dokładnie ścieżki klasy (a) zapamiętane w Fazie 0** (plan `docs/plans/<plik>-plan.md`, `docs/plans/<slug>-figma/`, `docs/operator/<slug>-przygotowanie.md`, ew. zmieniony requirements doc z `origin:`) + plan techniczny, jeśli dopisałeś do niego fazy / poprawki E2E w Fazie 1–2. Dodawaj wyłącznie wylistowane ścieżki — bez blanket `git add docs/plans/` i bez `git add -A`.
