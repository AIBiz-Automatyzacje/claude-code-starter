...
| Workflow | Co robi |
|----------|---------|
| `dev-autopilot-wf` | Autonomiczny pipeline: bootstrap (stan z `.autopilot-state.json`) → per faza (execute → review + adversarial verify → fix + targeted verify P1) → compound → **compound-refresh (scoped)** → complete → **telemetria** (1 linia JSONL do globalnego `~/.claude/telemetry/autopilot-runs.jsonl` — dane do strojenia progów: liczniki P1/P2/P3, wynik fixa, **metryki routingu/dedupu/verify per faza**, tokeny; metryki utrwalane w stanie, więc resume ich nie gubi). |
...
| `dev-docs-review-wf` | Review jednej fazy: context-packager (mapa zmian + **flagi warstw**) → **routing domenowy** (rdzeń `security`/`spec-compliance`/`simplicity`/`test-coverage` zawsze; `performance`/`architecture`/`typescript`/`e2e` tylko gdy ich domena jest w fazie obecna; brak flag = pełny skład) → do 8 reviewerów równolegle → dedup 2-przebiegowy (JS + semantyczny Haiku) → adversarial verify P1/P2 → scribe zapisuje raport + sekcję **`## Przebieg review`** + bookkeeping checkboxów `Weryfikacja:` → severity gate. |
