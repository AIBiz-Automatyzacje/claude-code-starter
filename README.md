...
/dev-ideate → /dev-brainstorm → /dev-prep → /dev-plan → /dev-docs → [ dev-autopilot-wf ] → gotowe
  (pomysły)     (CO budować)   (CZEGO       (JAK)      (struktura)   (cały pipeline auto)
                                potrzeba)
...
**`/dev-prep`** *(opcjonalny — nowy etap z UI)* — **zamówienie na materiały wejściowe** (**CZEGO POTRZEBA, żeby zaplanować**). Czyta etap zbiorczego dokumentu wymagań (`docs/brainstorms/mvp-requirements.md#etap-17`), skanuje repo (istniejące route'y, `docs/DESIGN.md`, `SPEC.md`, nazwy zmiennych w `.env.example`) i wypisuje cztery listy: **makiety do zaprojektowania** (nazwa ekranu w kebab-case, stany, breakpointy, wymagania — z pustym polem `URL Figma:` do wklejenia), konta/klucze/dostępy, assety i treści, decyzje produktowe do domknięcia. Nie planuje — zero IU, zero architektury. → `docs/operator/<slug>-przed-planem.md`
- **Kiedy:** startujesz etap dotykający UI i chcesz zamówić makiety, zanim usiądziesz do planowania; etap wymaga kont zewnętrznych, których zdobycie zajmie dni. **Kiedy nie:** bugfix, tech-debt, zmiana czysto backendowa — wtedy wprost `/dev-plan`.
- **Sprzężenie z `/dev-plan`:** ten sam `<feature-slug>`; gdy wszystkie ekrany mają wklejone URL-e Figmy, `/dev-plan` **nie pyta o Figmę** — fetchuje wprost z listy (1.6 krok C). Nieodhaczone pozycje kont/assetów przechodzą do sekcji „Wymagania wstępne operatora" (3.7), a odhaczone nie są powtarzane.
...
├── operator/                 ← dokumenty dla człowieka: <slug>-przed-planem.md (z /dev-prep, PRZED planowaniem),
│                                <slug>-przygotowanie.md (z /dev-plan, PRZED autopilotem)
│                                i <data>-<zadanie>-smoke.md (z /dev-docs-complete, PO autopilocie)
