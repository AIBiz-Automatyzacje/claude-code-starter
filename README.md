...
| `dev-docs-review-wf` | Review jednej fazy: context-packager → routing reviewerów wg mapy zmian (rdzeń zawsze; perf/architektura warunkowo na małych fazach) → do 8 reviewerów równolegle → dedup 2-przebiegowy (JS + semantyczny Haiku) → adversarial verify P1/P2 → scribe zapisuje raport + bookkeeping checkboxów `Weryfikacja:` → severity gate. |
